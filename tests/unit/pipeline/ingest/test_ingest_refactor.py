...
#import pickle
import context
import ingest_refactor as ingest
...
class IngestOptTest(unittest.TestCase):
    ...
    def test_sigterm_handler(self):
        ...
        subprocess.run(['pkill', '-f', 'python3 test_ingest_refactor.py'])
    ...
    @patch('ingest_refactor.ImageStore')
    @patch('ingest_refactor.Cluster')
    @patch('ingest_refactor.Producer')
    @patch('ingest_refactor.Consumer')
    def test_setup(self,
            mock_consumer,
            mock_producer,
            mock_cluster,
    ...
    @patch('ingest_refactor.executeLoadAsync')
    ...
    @patch('ingest_refactor.Ingester._insert_cassandra_multi')
