...
The root of the tree is [here](https://github.com/lsst-uk/lasair-examples/blob/main/).
They are described below.
----

### Inspect object
- [Examine an Object](https://github.com/lsst-uk/lasair-examples/blob/main/notebooks/examine_object.ipynb): Learn about the data associated with a Lasair object.
...
### Brief Notebooks
- [Cone search](https://github.com/lsst-uk/lasair-examples/blob/main/notebooks/cone.ipynb): The cone search method of the API finds objects within a cone, i.e. a point in the sky and radius in arcseconds. It can return:
    - A count of the number in the cone
    - The nearest of those in the cone
    - All of those in the cone

- [Object API method](https://github.com/lsst-uk/lasair-examples/blob/main/notebooks/object.ipynb): For a given object, identified by its diaObjectId, the object API call has two additional arguments:
    - `lasair_added`: True if you want the attributes that Lasair adds, as well as the Sherlock association, TNS crossmatch, and links to all the image cutouts.
...
- [Database query](https://github.com/lsst-uk/lasair-examples/blob/main/notebooks/query.ipynb): This notebook runs a Lasair filter using three strings: 'selected', 'tables', and 'conditions', as in the web interface. In this case we just fina a few objects that have a Sherlock host association.

- (Two Sherlock methods)[https://github.com/lsst-uk/lasair-examples/blob/main/notebooks/sherlock_api.ipynb]: Demonstrates the two Sherlock calls: one by diaObjectId and the other by sky position ra, dec.

- [Query with watchlist](https://github.com/lsst-uk/lasair-examples/blob/main/notebooks/watchlist.ipynb): This notebook runs a Lasair filter that includes a watchlist.

- [Consume Kafka](https://github.com/lsst-uk/lasair-examples/blob/main/notebooks/consume_kafka.ipynb): Shows how to consume Kafka from your active filter.

- [Consume Kafka and plot](https://github.com/lsst-uk/lasair-examples/blob/main/notebooks/consume_kafka_plot.ipynb): Shows how to consume Kafka and print the results. You will need to change the topic to the one listed in the webpage for your active filter.


### Scripts
- [Consume Kafka](https://github.com/lsst-uk/lasair-examples/blob/main/scripts/kafka.py): Small python code to consume (read) a kafka stream from an active filter

- [Annotate](https://github.com/lsst-uk/lasair-examples/blob/main/scripts/annotator.py): Small python code to consume (read) a kafka stream from an active filter that has the `lightcurve_lite` mode, and plot the lightcurve.

### Lasair Features
Shows how some of the Lasair added value is made.

- [Jump Feature](https://github.com/lsst-uk/lasair-examples/blob/main/features/jump.ipynb):
This simple feature is to filter out lightcurves with a significant change in brightness. The mean and standard deviation are computed from the time between 70 days ago and 10 days ago. Then for each band, the difference between the latest flux and the mean is divided by the standard deviation: the maximum over bands is `jump1`. The same maximum is computed over the other 5 flux bands: this is `jump2`. If there is a true jump in brightness, we can expect both jump1 and jump2 to be larger than several sigma.

- [Extinction and galactic latitude](https://github.com/lsst-uk/lasair-examples/blob/main/features/milky_way.ipynb): Calculation of the extinction E(B-V) and the galactic latitude. To run this notebook, you will need to install the dustmaps package that puts a 64 Mbyte file on your system.

- [Effective temperature from Pairs](https://github.com/lsst-uk/lasair-examples/blob/main/features/pair_analysis.ipynb): The Rubin cadence includes the same object in different wavebands only ~30 minutes apart, so it is possible to separate rapid brightening from colour. While the actual colour is reported (eg magnitude difference is 0.99 between filters g and r), this is also converted to an effective temperature using the blackbody model.
